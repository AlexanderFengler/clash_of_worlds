...
def run_subj_model(df_subj, model_idx, mode,
    ...
    Arguments:
        ...
        mode: str <default='navarro'>
            Specifies whether to run the hddm via analytical likelihood ('navarro') or via LANs ('lan')
    ...
    subj_idx = df_subj['subj_idx'].values[0]
    ...
    for chain in range(1, n_chains + 1, 1):
        ...
        if os.path.exists(model_file_name):
            ...
            data_file_name = save_folder + '/' + mode + '_df_chain_' + str(chain) + '_' + model_idx + '_subj_' + str(subj_idx) + '.pickle'
...
if __name__ == "__main__":
    ...
    if not args.parallel:
        ...
        for subj_idx in subj_ids:
            ...
            df_subj = data[subj_idx]['dataset']
            print(df_subj)

            model_tmp = run_subj_model(df_subj, model_idx = args.model_idx, mode = args.mode,
                                       include = ['z'],
        ...
        par_args = [data[subj_idx]['dataset'] for subj_idx in subj_ids]
        
        prepped_run_subj_model = partial(run_subj_model, 
        ...
        with Pool(processes = n_cpus) as pool:
            pool_out = pool.map(prepped_run_subj_model, par_args)
